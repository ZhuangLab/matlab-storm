 ...
 function [pars_nm, fresx2, fresy2] = ComputeZCalibration(varargin) % (bead_path,froot,ini,nooverwrite)
%--------------------------------------------------------------------------
% ComputeZCalibration('daxfile',daxfile,'parsfile',parsfile)
%
% ComputeZCalibration('froot',froot,'bead_path',bpath,'parsfile',parsfile)
%
%--------------------------------------------------------------------------
% Inputs
% daxfile / string / ''
%               -- full path daxfile name of the movie being analyzed.
%               Alternatively specify bead_path and froot (see below)
% BeadPath / string / 'C:/data/Beads/'
%                -- location of bead data for z-calibration and chromatic
%                calibration
% froot / string / e.g. '750_zcal'
%               -- string that contains the root of the data movie used for
%               the dax/bin/off files.
% parsfile / string / e.g. 'C:/data/Beads/750_zcal.ini'
%               -- full path and name of pars file to use as a stem. All
%               features of this file will be reproduced in the output file
%               ini_nm except the 'z calibration expression=...'
%               if an xml file, the appropriate fields will be changed as
%               well.  
...
% corrtilt / logical / true
%               -- correct stage tilt in computing z-calibration (strongly
%               recommended). 
% NewParsRoot / string / _zpars
%               -- will get appended to the end of the new parameter file
%               generated by this script.  
% MaxError / double / 60 
%               -- maximum distance from fitted curve that a bead can be in
%               order to be considered valid and used in computing new fit.
%                If original estimates are way off, you might need to
%                increase this in order to get a good first fit.  
%--------------------------------------------------------------------------
% Outputs (optional)
% pars_nm / string / 'C:/data/froot_zpars.ini'
%               -- name of new parameter file written by analysis script
...
if nargin > 0
    ...
    for parameterIndex = 1:parameterCount,
        ...
        switch parameterName
            ...
            case 'corrtilt'
                corrtilt = CheckParameter(parameterValue, 'boolean', 'corrtilt');
            case 'NewParsRoot'
...
%% Hardcoded Variables
%--------------------------------------------------------------------------
zwin_in = MaxError; % nm max deviation from 
zwin = zwin_in;
new_zmax = 700; % 
zmin = -600; %nm
zmax = 600; %nm
...
% Plot current fit
if nooverwrite == 1
    ver = dir([bead_path,'\fig_',froot,'wxy*.png']);
    ver = length(ver)+1;
else
    ver = '';
end
...
if nooverwrite == 1
    ver = dir([bead_path,'\fig_',froot,'wxfit*.png']);
    ver = length(ver)+1;
else
    ver = '';
end
...
if nooverwrite == 1
    ver = dir([bead_path,'\fig_',froot,'wyfit*.png']);
    ver = length(ver)+1; 
else
    ver = '';
end
