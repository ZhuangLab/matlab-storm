...
dlg_title = 'Step 3 Pars: Filter Clusters';  num_lines = 1;
    ...
    if notCancel
        CC{handles.gui_number}.pars3.boxSize = str2num(Opts{1}); %#ok<*ST2NM> % for 160npp, 16 -> 10nm boxes
        CC{handles.gui_number}.pars3.maxsize = str2num(Opts{2}); % 1E4 at 10nm cluster_scale, 1.2 um x 1.2 um 
        CC{handles.gui_number}.pars3.minsize = str2num(Opts{3}); % eg. minsize is 100 10x10 nm boxes.  400 is 200x200nm
        CC{handles.gui_number}.pars3.mindots = str2num(Opts{4}); % min number of localizations per STORM dot
        CC{handles.gui_number}.pars3.startFrame = str2num(Opts{5});   
        CC{handles.gui_number}.pars3.mindensity = str2num(Opts{6});   
    end